...
public:
  ...
  SubscribeAndPublish()
  {
    ...
    pub_ = n_.advertise<ackermann_msgs::AckermannDriveStamped>("/vesc/low_level/ackermann_cmd_mux/input/teleop", 10, true);
  ...
  void callback(const geometry_msgs::Twist &cmd_vel_buffer)
  {
    ...
    if(abs(cmd_vel_buffer.angular.z - 90 ) > 0)
        cmd_vel.angular.z = (cmd_vel_buffer.angular.z - 90.0) / 120.0;
    else
        cmd_vel.angular.z = (cmd_vel_buffer.angular.z - 90.0) / 120.0;*/
    // printf("%f",cmd_vel.linear.x);
    // printf("%f",cmd_vel.angular.z);
    ack.drive.speed = (cmd_vel_buffer.linear.x - 1500.0) / 100.0;
    if(abs(cmd_vel_buffer.angular.z - 90 ) > 0)
        ack.drive.steering_angle = (cmd_vel_buffer.angular.z - 90.0) / 120.0;
    else
        ack.drive.steering_angle = (cmd_vel_buffer.angular.z - 90.0) / 120.0;
    pub_.publish(ack);
